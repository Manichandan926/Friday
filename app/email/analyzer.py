...
SYSTEM_PROMPT = """You are an email analysis sub-system for the FRIDAY AI assistant.
...
class EmailAnalyzer:
    def __init__(self):
        try:
            ...
            logger.error(f"Failed to load default LLM provider for email analysis: {e}")
            self.provider = None

    async def analyze(self, sender: str, subject: str, body: str) -> Optional[Dict[str, Any]]:
        """Invokes the default LLM to parse raw email body contents and return metadata dictionary."""
        if not self.provider:
            logger.error("LLM Provider is not loaded. Cannot run email analyzer.")
            return None
        ...
        try:
            logger.info("Submitting email to LLM for metadata analysis...")
            ...
            logger.error(f"LLM returned invalid JSON payload for email: {e}. Raw: {raw_response}")
            return None
        except Exception as e:
